...
| `next-best-practices` | `.agents/skills/next-best-practices/` | Writing or reviewing Next.js code: file conventions, RSC boundaries, data patterns, async APIs, metadata, error handling, route handlers, image/font optimization, bundling. Marked `user-invocable: false` — load on-demand when working on these topics; don't auto-route from the user prompt. |
| `next-cache-components` | `.agents/skills/next-cache-components/` | Next.js 16 Cache Components — PPR, `use cache` directive, `cacheLife`, `cacheTag`, `updateTag`. |
